<body style= "background-color: #f2f5fa;">
    <div class="about-main-cotent">
        <div class="container">
            <div class="row">
                <div class="col-12 col-lg-7 ">
                    <div class="title">
                        <p >ABOUT US</p>
                        <h5>“Our mission is to help job seekers <strong class="subtitle">grow careers</strong> . We love helping people stand out in their job search and get hired faster.”</h5>
                    </div>
                </div>
                <div class="col-12">
                    <div class="row">
                        <div class="col-12 col-md-6">
                          <div class="img">
                            <img src="./images/office1-4b814999eddc7918f5bdfabe2ad44acf294bb9ddfaba31cb16650aa9d086a1b5.jpg" class="img-fluid" alt="">
                          </div>
                        </div>
                        <div class="col-12 col-md-6">
                           <div class="img">
                                <img src="./images/office2-55aee052b07736a1f1531b732a7d0fabb573acbf9929daaeea9aa1c0a15c4c80.jpg" class="ig-fluid" alt="">
                           </div>
                        </div>
                    </div>
                </div>
            </div>
           
                
            </div>
        </div>          
    </div>
</body>
